...
소득 단독으로는 설명되지 않던 이탈이 **계약형태와 교차**하자 46.75% vs 1.9%로 갈림.
...
**해양 폐기물 정책 인과분석** (2인 팀, 2025-2학기) — Sentinel-2 위성영상 + MARIDA 라벨로
`Debris Ratio` 지표를 직접 설계하고, 유사 유역 대조군에 이중차분(DID) 적용.
구름 14.02% · 해양쓰레기 0.41%라는 관측 불균형을 지표 재정의로 처리.
결과 β₃ = −0.095 (p = 0.636)로 **유의하지 않음** — 법 제정과 물리적 개선 사이의 간극을 확인.

---

### 기술

**인과추론·계량** DID · 고정효과 패널 · 공간데이터(GeoPandas) · 시계열 파운데이션 모델(Chronos, TimesFM)
